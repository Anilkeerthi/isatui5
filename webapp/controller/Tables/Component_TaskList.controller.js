sap.ui.define([
  ...
  return Controller.extend("com.isat.isatui5.controller.Tables.Component_TaskList", {
    ...
    //open a NEW Dialog
    onComponent_TaskListNew: function () {
      // Reset the ComboBoxes
      this.byId("TasklistDialogNewComboBox").setSelectedKey("");
      this.byId("StageDialogNewComboBox").setSelectedKey("");
      this.byId("component_TaskListDialogNew").open();
    },

    // Close the NEW dialog 
    onClosecomponent_TaskListDialogNew: function () {
      this.byId("component_TaskListDialogNew").close();
      this.onClearcomponent_TaskListDialogNew();
      this._refreshTable();
    },

    //Clear the all fields of NEW Dialog
    onClearcomponent_TaskListDialogNew: function () {
      this.byId("TasklistDialogNewComboBox").setValue("");
      this.byId("StageDialogNewComboBox").setValue("");

    },

    //refresh the DDData table
    _refreshTable: function () {
      var oTable = this.byId("idComponent_TaskList");
    ...
    onSavecomponent_TaskListDialogNew: function () {

      var sTasklist_id = this.byId("TasklistDialogNewComboBox").getSelectedKey();
      var sStage_id = this.byId("StageDialogNewComboBox").getSelectedKey();


      // Check for missing fields
      if (!sTasklist_id && !sStage_id) {
        MessageToast.show("Both Task List Name and Stages Name are mandatory.");
        return;
      } else if (!sTasklist_id) {
        MessageToast.show("Task List Name is mandatory.");
        return;
      } else if (!sStage_id) {
        MessageToast.show("Stages Name is mandatory.");
        return;
      }

      // Ensure addType_id is handled as a proper integer
      sTasklist_id = sTasklist_id.replace(/,/g, '');  // Remove any commas if present
      sTasklist_id = Number(sTasklist_id);  // Convert to a plain integer

      sStage_id = sStage_id.replace(/,/g, '');  // Remove any commas if present
      sStage_id = Number(sStage_id);  // Convert to a plain integer

      // Get the OData model
      let oModel = this.getView().getModel();

      // Bind the list (Assuming you're binding to "/DDData" entity set)
      let oBindList = oModel.bindList("/Component_TaskList");

      // Create the new entry using the bindList create method
      oBindList.create({
        tasklist_id: { "autoid": sTasklist_id },
        stage_id: { "autoid": sStage_id }
      }
      );
      // Close the dialog after saving
      this.onClosecomponent_TaskListDialogNew();
    },
    ...
    onClosecomponent_TaskListDialog: function () {
      this.byId("component_TaskListDialog").close();
    },

     // Delete functionality for ACTIONS DialogBox
     onDeletecomponent_TaskListDialog: function () {
      // Get the dialog model
      var oDialogModel = this.getView().getModel("dialogModel");
      var oData = oDialogModel.getData();
      ...
      MessageBox.confirm("Are you sure you want to delete this item?", {
        ...
        onClose: function (oAction) {
          if (oAction === MessageBox.Action.YES) {
            ...
            that.onClosecomponent_TaskListDialog();
          }
        }
      });
    },
