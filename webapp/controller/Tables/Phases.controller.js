sap.ui.define([
    ...
    return Controller.extend("com.isat.isatui5.controller.Tables.Phases", {
    ...
    onToggleEdit: function () {
     ...
     onDeletePress: function () {
      ...
      if (oData.deleteButtonText === "Delete") {
        ...
        this._deletePhases(oData.autoid);
      } else {
 ...
 _saveChanges: function (oData) {
  ...
  let oBindList = oModel.bindList("/Phases");
  ...
  oBindList.filter(aFilter).requestContexts().then(function (aContexts) {
    if (aContexts && aContexts.length > 0) {
      ...
      aContexts[0].setProperty("phase_name", oData.phase_name);
...
onCloseDialogAction: function () {
  this.byId("PhasesDialog").close();
},

// Refresh table functionality
_refreshTable: function () {
  var oTable = this.byId("idPhases");
 ...
 onPhasesNew: function () {
  // Open the dialog
  this.byId("PhasesDialogNew").open();
},

onSavePhasesNew: function () {
  var sPhaseName = this.byId("PhasesPhaseName").getValue();
 
  if (!sPhaseName) {
    sap.m.MessageToast.show("PhaseName is mandatory.");
    return;
  } 
  let oModel = this.getView().getModel();
  let oBindList = oModel.bindList("/Phases");
...
onClearDialog: function () {
  ...
  this.byId("PhasesPhaseName").setValue("");
...
onCloseDialog: function () {
  // Close the dialog
  this.byId("PhasesDialogNew").close();
