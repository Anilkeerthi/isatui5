sap.ui.define([
    ...
    return Controller.extend("com.isat.isatui5.controller.Tables.Roles", {
...
onDeletePress: function () {
  ...
  if (oData.deleteButtonText === "Delete") {
    ...
    this._deleteRoles(oData.autoid);
  } else {
...
_saveChanges: function (oData) {
  ...
  let oBindList = oModel.bindList("/Roles");
  ...
  oBindList.filter(aFilter).requestContexts().then(function (aContexts) {
    if (aContexts && aContexts.length > 0) {
      ...
      aContexts[0].setProperty("roleid", oData.roleid);
...
onCloseDialogAction: function () {
  this.byId("RolesDialog").close();
},

// Refresh table functionality
_refreshTable: function () {
  var oTable = this.byId("idRoles");
...
onRolesNew: function () {
  // Open the dialog
  this.byId("RolesDialogNew").open();
},

onSaveRolesNew: function () {
  var sRoleID = this.byId("RolesRoleID").getValue();
  var sName = this.byId("RolesName").getValue();
  var sDescription = this.byId("RolesDescription").getValue();
  
  if (!sRoleID && !sName && !sDescription) {
    sap.m.MessageToast.show("All fields are mandatory.");
    return;
  } else if (!sRoleID) {
    sap.m.MessageToast.show("RoleID is mandatory.");
    return;
  }else if (!sName) {
...
onClearDialog: function () {
  ...
  this.byId("RolesRoleID").setValue("");
  this.byId("RolesName").setValue("");
  this.byId("RolesDescription").setValue("");
...
onCloseDialog: function () {
  // Close the dialog
  this.byId("RolesDialogNew").close();
